...
```{r}
#| label: square-multiply-it-fcn-run
#| echo: true
...
```{r eval-data-1, eval = F}  
eval_data_1 <- expand.grid(
...
By calling the same rows over and over again, you can replicate the same rows multiple times:

```{r }
field_data[c(1, 1, 1, 2, 2, 2), ]
```

Hard-coding the vector, 
...
```{r }
rep(1:nrow(field_data), each = length(N_seq))
```
]
...
```{r }
field_data %>% 
  slice(rep(1:nrow(.), each = length(N_seq)))
```

This approach does not create plots that do not exist unlike the expand-everything approach.
