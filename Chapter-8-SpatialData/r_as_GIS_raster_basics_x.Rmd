...
Go [here](https://www.dropbox.com/sh/iih8icftc4xa7nm/AABfQj82A-i8GWaHgHngLMQAa?dl=0) and download all the files.
...
`terra` offers only one obejct class for raster data: `SpatRast`
...
Let's take a look at a `SpatRaster` object
```{r, cache = FALSE}
(
  drone_blue_sr <- terra::rast("Data/Annos_062718_RE_transparent_reflectance_blue.tif")
)
```

Yes, the information presented here is very similar to what we saw earlier with `Raster`* object.

---
class: middle

But, `SpatRaster` object can have multiple layers like `RasterLayer` and `RasterBrick`. 
...
Sometimes, it is necessary to convert a `SpatRaster` to a `Raster`* object because there are many packages that accepts only `Raster`* objects. Fortunately, it is easy to do this.

+ from `SpatRaster` to `Raster`*

```{r}
as(drone_blue_sr, "Raster")
```
...
# Read raster file(s) with `raster`
...
(
  drone_blue_green <- raster::stack(files_list)
)
```
...
# Read raster file(s) with `terra`

You can use `rast()` to read raster data.

```{r read_no_eval_terra}
drone_blue_sr <- terra::rast("./Data/Annos_062718_RE_transparent_reflectance_blue.tif")
```

You can read multiple raster data files into one `SpatRast` object as we saw earlier.
...
# Write raster files with `raster`
...
# Write raster files with `terra`

You can write a `SpatRast` object using `terra::writeRaster()` (yes, it is exactly the same function name with `raster`).

.content-box-green[**Example**]

```{r, eval = F}
terra::writeRaster(drone_green, "./Data/export_example_green_using_terra.tif", format = "GTiff", overwrite = TRUE)
```

However, the key difference between the `raster` and `terra` is the `terra` writes the file much faster.
...
Read "NIR.tif" and "RED.tif", and name them `NIR` and `RED`. You can use either `raster` or `terra`. 
...
```{r include = F}
NIR <- terra::rast("./Data/NIR.tif")
RED <- terra::rast("./Data/RED.tif")
```
...
+ You can do basic arithmetic operations using raster layers as long as they share the same spatial extent and resolution (both `raster` and `terra`). 
...
```{r ndvi-calc, include = F, cache = TRUE}
NDVI <- (NIR - RED) / (NIR + RED)
```

Then, use the following code to visualize NDVI.
...
.right5[
```{r echo = F, cache = TRUE, out.width="100%"}
tm_shape(NDVI) +
...
.right5[
```{r plot_stack_run, echo = F, out.width = "100%"}
plot(drone_blue)
```
]
...
```{r tmap-static-raster, eval = F, out.width = "100%"}  
tm_shape(drone_blue) +
  tm_raster()
```
]
...
.right5[
```{r tmap-static-raster-f, ref.label = "tmap-static-raster", echo = F, out.width = "100%"}  
